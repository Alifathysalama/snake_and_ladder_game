...
main ()
{
  ...
  printf
    ("now collide your name's first letter with the given dot using the keys\n\n W for up \n S for down\n A for left\n D for right\n");
  gotoxy (x, y);
	...
	if (choice == 2)	//if true enter the game
	  {
	    ...
	    printf
	      ("winner will be the one who gets to 100 first that is minimum number of dice throws");
	    do
	      {
		startmin = clock ();
...
in worst case it will be 0.001 second as the cpu will run on all cases*/
      ...
      printf
		  ("\nchecking if there is another player turn and storing this turn's record %f seconds to execute \n",
		   timemin);
    }
...
void
push (int x)			/* push the count of each player in the top of the stack  */
{
  if (top == size)
    {
...
void
insert1 (int x)			/* store the count of each player in the queue  */
{
  if (rear == size1)
    {
      printf ("winner can be decided only between 3 players");
...
void
display ()			/* display the player with least number of count to win the game   */
{
  if (top == 0)
    {
      printf ("player 1");
...
void
display1 ()			/*  display the player with most number of count to lose the game      */
{
  if (rear == 0)
    {
      printf ("player 1");
